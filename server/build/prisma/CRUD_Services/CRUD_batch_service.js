...
exports.cancelBatchById = exports.getBatchStepById = exports.getBatchPage = exports.updateBatchById = exports.deleteBacthById = exports.createBatch = exports.getBatchById = exports.getAllBatches = exports.updateFeedbackBatchStep = exports.getBatchStatus = void 0;
const client_1 = require("@prisma/client");
const pagination_1 = require("../pagination");
const date_fns_tz_1 = require("date-fns-tz");
const CRUD_ingredient_service_1 = require("./CRUD_ingredient_service");
...
const getAllBatches = async () => {
    try {
        ...
        const result = validate.map((batch, index) => ({
            ...batch,
            status: (0, exports.getBatchStatus)(batch.batchSteps, batch.isCancelled ?? false),
        }));
        return {
            message: "Thành công",
            data: result,
    ...
    try {
        ...
        const batchIngredients = data.batchIngredients.map((bi) => ({
            ...
            ingredient: {
                id: bi.ingredientId,
                name: bi.ingredient
                    ? `${bi.ingredient.name}${bi.ingredient.isDeleted ? " (đã bị xóa)" : ""}`
        ...
        const status = (0, exports.getBatchStatus)(data.batchSteps, data.isCancelled ?? false);
        return {
            message: "Thành công",
            data: {
                id: data.id,
...
const getBatchStepById = async (id) => {
    try {
...
exports.getBatchStepById = getBatchStepById;
// getBatchStepById(1)
//   .then(() => console.log("Thành công"))
//   .catch(() => console.log("Thất bại"));
const getBatchStatus = (batchSteps, isCancelled) => {
    if (isCancelled)
        return "Đã hủy";
    const now = new Date();
...
const createBatch = async (beerName, volume, notes, recipeId, createdById) => {
    try {
        // 1. Tạo mã tự động
        const vnNow = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" }));
        const datePrefix = (0, date_fns_tz_1.format)(vnNow, "ddMMyy");
        ...
        const activeIngredients = recipe.recipeIngredients.filter((ri) => ri.ingredient && !ri.ingredient.isDeleted);
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const batchSteps = recipeSteps.map((step) => {
                ...
                const scheduledEndAt = new Date(startedAt.getTime() + step.durationMinutes * 60 * 1000);
        ...
        const batchStatus = (0, exports.getBatchStatus)(fullBatch.batchSteps, fullBatch.isCancelled ?? false);
        ...
        for (const bi of fullBatch.batchIngredients) {
            ...
            // @ts-ignore
            bi.ingredient.status = await (0, CRUD_ingredient_service_1.getIngredientStatus)(bi.ingredient.quantity, bi.ingredient.lowStockThreshold);
        }
        return {
...
const updateFeedbackBatchStep = async (id, updateData) => {
    try {
...
exports.updateFeedbackBatchStep = updateFeedbackBatchStep;
...
const updateBatchById = async (id, updateData) => {
    try {
        const existing = await prisma.batch.findUnique({
            where: { id },
        });
...
const deleteBacthById = async (id) => {
    try {
        const existing = await prisma.batch.findUnique({
            where: { id },
        });
...
exports.deleteBacthById = deleteBacthById;
const cancelBatchById = async (id) => {
    try {
        const existing = await prisma.batch.findUnique({
            where: { id },
        });
...
exports.cancelBatchById = cancelBatchById;
const getBatchPage = async (page, limit) => {
    return (0, pagination_1.paginate)({
        ...
        enhanceItem: async (batch) => {
            const status = (0, exports.getBatchStatus)(batch.batchSteps, batch.isCancelled);
            return {
                ...batch,
                recipe: {
                    ...batch.recipe,
...
exports.getBatchPage = getBatchPage;
