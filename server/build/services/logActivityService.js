...
exports.compareAndLogChanges = void 0;
// utils/compareAndLog.ts
const logActivity_1 = require("../prisma/logActivity");
const compareAndLogChanges = async (oldData, newData, fieldsToCompare, entity, entityId, entityLabel, // ví dụ: oldData.name
userId) => {
    ...
    for (const field of fieldsToCompare) {
                    ...
                    if (typeof oldItemValue === "object" &&
                        ...
                        for (const subKey of Object.keys(newItemValue)) {
                            ...
                            if (oldSubValue instanceof Date || newSubValue instanceof Date) {
                                ...
                                if (oldTime !== newTime) {
                                    logs.push(`- ✏️ **${formatFieldName(field)} #${i + 1} > ${formatFieldName(key)}.${subKey}**\n` +
                        ...
                        logs.push(`- ✏️ **${formatFieldName(field)} #${i + 1} > ${formatFieldName(key)}**\n` +
                            `  - Trước: \`${oldItemValue}\`\n` +
                            `  - Sau:   \`${newItemValue}\``);
                    }
                }
        ...
        // Trường đơn giản
            ...
            if (oldTime !== newTime) {
                logs.push(`- 🕒 **${formatFieldName(field)}**:\n  - Trước khi cập nhật: \`${formatDate(oldValue)}\`\n  - Sau khi cập nhật: \`${formatDate(newValue)}\``);
            }
        }
        else if (oldValue !== newValue) {
            logs.push(`- ✏️ **${formatFieldName(field)}**:\n  - Trước khi cập nhật: \`${formatFieldName(oldValue)}\`\n  - Sau khi cập nhật: \`${formatFieldName(newValue)}\``);
        }
    }
    if (logs.length > 0) {
        const formattedLogs = logs
            ...
            .join("\n");
        const fullLogMessage = `Cập nhật ${formatFieldName(entity)} "${entityLabel}":\n` +
            `${formattedLogs}\n` +
            `\nVào: *${logUpdateDate}*`;
        await (0, logActivity_1.logActivity)("update", formatTableName(entity), entityId, fullLogMessage, userId);
    }
    return logs.length > 0;
};
exports.compareAndLogChanges = compareAndLogChanges;
...
function formatTableName(field) {
    switch (field) {
        case "Recipe":
            return "Công thức";
        case "Batch":
